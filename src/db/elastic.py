...
def get_pagination_param(page_number: int, page_size: int) -> tuple:
    start_number = (page_number - 1) * page_size
    end_number = page_number * page_size
...
class ElasticMoviesStorageService(BaseFilmStorageService):
    ...
    async def get_film_list(self, sort: str, page_number: int, page_size: int, filter_genre: str):
        ...
        if filter_genre:
            s = (
                ...
                .query(
                    ...
                    should=[Q("nested", path="genre", query=Q("match", genre__id=str(filter_genre)))],
                )
    ...
    async def search_film(self, query: str, page_number: int, page_size: int) -> list:
        start_number, end_number = get_pagination_param(page_number, page_size)
        s = Search(index=self.index).query("multi_match", query=query, fuzziness="auto")[
    ...
    async def get_person_films_by_person_id(self, person_id: UUID4):
        search_query = Search(index=self.index).query(
            "bool",
            minimum_should_match=1,
            should=[
